...
public class FileManager {
	 ...
	 * Directorio base
	 */
	public final static String BASE_DIRECTORY = "/Editors";

	/**
	 * Directorio de imagenes
	 */
	public final static String IMG_DIRECTORY = "Images";

	/**
	 * Directorio de scripts
	 */
	public final static String SCR_DIRECTORY = "Scripts";

	/**
	 * Directorio de metainformación
	 */
	public final static String META_DIRECTORY = "Meta";

	/**
	 * Nombre del archivo de metainformacion
	 */
	public final static String META_INF = "meta.txt";

	/**
	 * Directorio con el metamodelo
	 */
	public final static String META_MODEL_DIRECTORY = "Model";

	/**
	 * Directorio con el archivo picture
	 */
	public final static String PICTURE_LANG_DIRECTORY = "Picture";

	/**
	 * Directorio para diagramas
	 */
	public final static String DIAGRAM_DIR = "Diagrams";
	...
	/**
	 * Crea el directorio base para el editor junto con los subdirectorios para
	 * utilizar
	 ...
	 */
	public synchronized void makeMeta(String path, String name, String author,
			String description, Date date) {
		PrintWriter writer;
		try {
			writer = new PrintWriter(path + "/" + META_DIRECTORY + "/"
					+ META_INF, "UTF-8");
	...
	/**
	 * Borra el contenido de un folder existente en el direcctorio de archivos
	 * del servidor
	...
	public synchronized void createScript(String path, String script) {
		PrintWriter writer;
		try {
			writer = new PrintWriter(path + "/" + SCR_DIRECTORY + "/"
					+ "editor.js", "UTF-8");
	...
	public synchronized String saveDiagram(String path, FileItem model) {
		...
		File F = new File(path + "/" + DIAGRAM_DIR + "/" + modelDir + "/"
				+ model.getName());
		try {
			model.write(F);
	 ...
	 */
	public synchronized void uploadGraphicalRepresentation(String path,
			FileItem pic) {
		File F = new File(path + "/" + PICTURE_LANG_DIRECTORY + "/"
				+ pic.getName());
		try {
			pic.write(F);
		} catch (Exception e) {
	...
	public synchronized void uploadMetamodel(String path, FileItem pic) {
		File F = new File(path + "/" + META_MODEL_DIRECTORY + "/"
				+ pic.getName());
		try {
			pic.write(F);
		} catch (Exception e) {
